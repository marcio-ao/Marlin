...
#include "../../Marlin.h"
  ...
  /**
   * We back away from the cube by various distances depending on how uncertain we are:
   *
  ...
  static void calibrate_toolhead(measurements_t &m, float uncertainty, uint8_t extruder);
  static void calibrate_all_toolheads(measurements_t &m, float uncertainty);
  ...
  #if HOTENDS > 1
    static void normalize_hotend_offsets();
    static void report_hotend_offsets();
  #endif
  static void park_above_cube(measurements_t &m, float uncertainty);
  static void probe_cube(measurements_t &m, float uncertainty);
  static void move_to(
    const AxisEnum a1,           const float p1,
    const AxisEnum a2 = NO_AXIS, const float p2 = 0,
    const AxisEnum a3 = NO_AXIS, const float p3 = 0
  );
  static float measure(const AxisEnum axis, const int dir, const bool stop_state, float *backlash_ptr,  const float uncertainty);
  ...
  /**
   * This class saves, changes and restores the backlash compensation
   * state for the scope in which it is defined.
   */
  ...
  /**
   * The following class saves, changes and restores the endstop
   * state for the scope in which it is defined.
   */
  ...
  /**
   * Performs a full auto-calibration routine:
   *
  ...
  static void calibrate_toolhead(measurements_t &m, float uncertainty, uint8_t extruder) {
    ...
    probe_cube(m, uncertainty);
    if(!fast) {
      #if ENABLED(BACKLASH_GCODE) && defined(CALIBRATION_MEASUREMENT_DEBUG)
    ...
    planner.synchronize();
    #if HAS_X_CENTER
      current_position[X_AXIS] += m.pos_error[X_AXIS];
      m.cube_center   [X_AXIS]  = m.true_center[X_AXIS];
      m.pos_error     [X_AXIS]  = 0;
    #endif
    #if HAS_Y_CENTER
      current_position[Y_AXIS] += m.pos_error[Y_AXIS];
      m.cube_center   [Y_AXIS]  = m.true_center[Y_AXIS];
      m.pos_error     [Y_AXIS]  = 0;
    #endif
    current_position  [Z_AXIS] += m.pos_error[Z_AXIS];
    m.cube_center     [Z_AXIS]  = m.true_center[Z_AXIS];
    m.pos_error       [Z_AXIS]  = 0;
    sync_plan_position();
  }
  ...
  /**
   * Moves to the exact center above the calibration cube
   *
   ...
   */
  static void probe_cube_side(measurements_t &m, const float uncertainty, const side_t side, const bool probe_top_at_edge = false) {
    AxisEnum axis;
    float dir;
  ...
  /**
   * Moves along axis in the specified dir, until the probe value becomes stop_state,
  ...
  float measuring_movement(const AxisEnum axis, const int dir, const bool stop_state, const bool fast) {
    ...
    const float mms   = MMM_TO_MMS(fast ? CALIBRATION_FAST_FEEDRATE : CALIBRATION_SLOW_FEEDRATE);
  ...
  /**
   * Moves along axis until the probe is triggered. Will return the axis value at
   * that point and returns toolhead to starting point.
   *
   *   axis               in     - Axis along which the measurement will take place
   *   dir                in     - Direction along that axis (-1 or 1)
   *   stop_state         in     - Move until probe pin becomes this value
   ...
   */
  float measure(const AxisEnum axis, const int dir, const bool stop_state, float *backlash_ptr, const float uncertainty) {
    const bool fast = uncertainty == CALIBRATION_MEASUREMENT_UNKNOWN;
